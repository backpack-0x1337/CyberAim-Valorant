...
class Aimbot:
    ...
    def __init__(self, box_constant = 416, collect_data = False, mouse_delay = 0.0001, debug = False):
        ...
        self.model = torch.hub.load('ultralytics/yolov5', 'custom', path='lib/aimlab-200.pt', force_reload = True)
    ...
    def move_crosshair(self, x, y):
        if Aimbot.is_targeted(self):
            ...
            for rel_x, rel_y in Aimbot.interpolate_coordinates_from_center((x, y), scale):
                print("rel_x is : {}", rel_x)
                print("rel_y is : {}", rel_y)
                difX = int(rel_x)
                difY = int(rel_y)
